// Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see license.txt)
    ...
    ////////////////////////////////////////////////////////
    // 基础 instancing 设置
    // - UNITY_VERTEX_INPUT_INSTANCE_ID     顶点着色器输入/输出结构中声明instance ID的字段，
    // 用于每个着色器阶段中识别当前正在被处理的几何体实例
    // - UNITY_GET_INSTANCE_ID              （内部用）从输入结构中获取instace ID
    ...
    // - UNITY_GET_INSTANCE_ID              (Internal) Get the instance ID from input struct.
        ...
        // 函数可以直接访问的一个全局的instance ID 
        // 每一个多例化对象对应的instance id
        static uint unity_InstanceID;

        // Don't make UnityDrawCallInfo an actual CB on GL
        // 不要使UnityDrawCallInfo成为GL（OpenGL）上实际的CB（CB -> CBUFFER？）
        // 在OpenGL中不定义在常量缓冲区中？
        #if !defined(SHADER_API_GLES3) && !defined(SHADER_API_GLCORE)
            UNITY_INSTANCING_CBUFFER_SCOPE_BEGIN(UnityDrawCallInfo)
        #endif
        /*
        Unity 3D 引入了用来提高渲染效率的批次化渲染（batch render）技术。
        此技术的基本原理就是通过将一些渲染状态（render state）一致的待渲染对象组成一组，
        一次性提交给 GPU 进行绘制，而不需要来回地设置【渲染状态】，这可以显著地节省绘制调用。
        很显然，使用了 GPU 多例化技术的待渲染对象的渲染状态基本是一致的。
        因此，对多例化的【顶点进行分批次时】，引擎底层会指明该批次的起始实例 id 及该批次有多少个实例 id。
        */
        // 当前渲染批次的起始实例，在实例数组中的数组索引值
        int unity_BaseInstanceID;
        // 当前渲染批次中的多例化对象的个数，如果是立体渲染，那么就是在双次渲染之前的对象实例个数
        int unity_InstanceCount;
        #if !defined(SHADER_API_GLES3) && !defined(SHADER_API_GLCORE)
            UNITY_INSTANCING_CBUFFER_SCOPE_END
        #endif

        // UNITY_GET_INSTANCE_ID，作用是获取到 UNITY_VERTEX_INPUT_INSTANCE_ID 宏所表示的当前渲染批次下的实例id
        #ifdef SHADER_API_PSSL
            #define DEFAULT_UNITY_VERTEX_INPUT_INSTANCE_ID uint instanceID;
            #define UNITY_GET_INSTANCE_ID(input)    _GETINSTANCEID(input)
        #else
            // 非 PlayStation 平台，本质就是一句代码
            // 用于每一个着色器阶段中识别当前正在被处理的几何体实例，
            // 只要声明并绑定了SV_InstanceID语义的实例id变量，
            // 在【输入组装阶段】渲染流水线会为其【赋值】上一个实例 id，
            // 然后在每一次【绘制调用时】，实例 id 变量的值将会【加 1】，
            // 当此值超过了 2^32−1 之后，将会重置为 0。
    ...
    /*
        - UNITY_SETUP_INSTANCE_ID：调用过程函数来设置实例数据，应该在顶点着色器/片段着色器的最开始使用，以便后续代码可以访问全局unity_InstanceID。
        - UNITY_TRANSFER_INSTANCE_ID：将实例ID从输入结构复制到输出结构。 用于顶点着色器。
    */
    #if defined(UNITY_INSTANCING_ENABLED) || defined(UNITY_PROCEDURAL_INSTANCING_ENABLED) || defined(UNITY_STEREO_INSTANCING_ENABLED)
        void UnitySetupInstanceID(uint inputInstanceID)
        {
            // 本函数执行unity_InstanceID的具体计算操作
            #ifdef UNITY_STEREO_INSTANCING_ENABLED
                    ...
                    /*
                    对于GLES3，我们必须对立体眼索引进行不同的计算，否则，Unity着色器编译器将发出bitfieldInsert（位域插入）函数。
                    bitfieldInsert需要支持glsl 400或更高版本。 因此，生成的glsl代码将无法在低端设备上编译。
                    通过更改计算立体眼索引的方式，我们可以帮助着色器编译器避免发出bitfieldInsert函数，
                    从而增加可以在其上运行立体实例化的设备数量。
                    */
                    ...
                    // 立体眼索引可以从实例ID中找出，最后一位，0或1
                    unity_StereoEyeIndex = inputInstanceID & 0x01;  
                    // 最后一位用来表示eyeIndex，所以这里加之前先移除最后一位
                    unity_InstanceID = unity_BaseInstanceID + (inputInstanceID >> 1);   
                #endif
            #else
                // 非立体渲染 -> 当前输入的顶点实例id + 当前渲染批次中在多例化对象数组中的起始位置
                unity_InstanceID = inputInstanceID + unity_BaseInstanceID;
            #endif
        }
        void UnitySetupCompoundMatrices();
        #ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
            // 如果启用表示自定义程序处理顶点实例化，那么就需要使用者自行定义一个名为的
            // UNITY_INSTANCING_PROCEDURAL_FUNC宏，以设置顶点的实例ID
            ...
            #endif
        #else
            // 如果未定义UNITY_PROCEDURAL_INSTANCING_ENABLED，则 DEFAULT_UNITY_SETUP_INSTANCE_ID 定义为
            // 调用UnitSetupInstanceID函数
    ...
    // 转调 DEFAULT_UNITY_SETUP_INSTANCE_ID
    ...
    // instanced property arrays
            ...
            #define UNITY_INSTANCED_ARRAY_SIZE  2 // minimum array size that ensures dynamic indexing 最小数组大小，可确保动态索引
        ...
        /// 这些语句的作用是定义一个着色器常量缓冲区，并定义一些着色器要使用的变量
