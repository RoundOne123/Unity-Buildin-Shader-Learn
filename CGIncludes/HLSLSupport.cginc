...
// **************** 和着色器编译器版本相关的宏  ****************
    ...
    // 如果要启用surface shader得检查步骤
        ...
        // Unity3D 使用一个开源库HLSLcc，将使用HLSL语法编写得着色器代码转成GLSL着色器代码或者面向苹果得Metal着色器代码
        // 从而使得采用 Cg/HLSL 语法编写的 Unity 3D ShaderLab 着色器代码能够通用于各个不同平台。
    ...
    // **************** END  ****************
        ...
        // 立体多例化渲染：在一次渲染管道上提交两份待渲染的几何体数据，减少DrawCall次数，提升渲染性能
    ...
    // **************** 利用宏消除各平台的语义差异性  ****************
    // UNITY_FRAMEBUFFER_FETCH_AVAILABLE： 用来表征目标硬件平台是否实现了【帧缓冲区撷取】这一功能
    // UNITY_FRAMEBUFFER_FETCH_ENABLED：假如实现了该功能，是否启用
    ...
    // **************** 消除SV_Target 和 SV_Depth 语义在各平台的差异性  ****************
    ...
    // Disable warnings we aren't interested in
        ...
        #pragma warning (disable : 3205) // conversion of larger type to smaller // 屏蔽把数据类型从大范围值转为小值时发生的警告
        ...
        #pragma warning (disable : 3206) // implicit truncation of vector type   // 把高精度的数据传给一个低精度类型的变量时，会带来隐式截断（implicit truncation）问题，屏蔽此警告
    #endif

    // **************** 消除fixed及其相关定义在非GLSL着色器语言下的平台差异性  ****************
    ...
    // **************** 统一着色器常量缓冲区的宏定义  ****************
        ...
        #define CBUFFER_START(name) ConstantBuffer name {       // 常量缓冲区的起始声明
        #define CBUFFER_END };                                  // 常量缓冲区的结束声明
    #elif defined(SHADER_API_D3D11)
        #define CBUFFER_START(name) cbuffer name {
        #define CBUFFER_END };
    #else
        // On specific platforms, like OpenGL, GLES3 and Metal, constant buffers may still be used for instancing
        // 在其他着色器语言，如 GLSL 中，constant buffer 这个概念的等价物是 uniform buffer object。
        // 但在面向 OpenGL 平台的 Cg/HLSL 代码中，则不用声明常量缓冲区，因此 CBUFFER_START 和 CBUFFER_END 都定义为空。
    ...
    // **************** END  ****************
        ...
        // 为了通过DX编译器传达此信息，我们将其包装到cbuffer中。
    ...
    /*
    深度纹理采样助手。
    在大多数平台上，您都可以对其进行采样，但是某些平台（例如PSP2）需要特殊处理。
    SAMPLE_DEPTH_TEXTURE（sampler，uv）：返回标量深度
    SAMPLE_DEPTH_TEXTURE_PROJ（sampler，uv）：投影的样本
    SAMPLE_DEPTH_TEXTURE_LOD（sampler，uv）：具有LOD级别的样本
    */
    ...
    /*
    UNITY_DECLARE_SHADOWMAP：声明一个阴影纹理
    UNITY_SAMPLE_SHADOW：进行采样操作的宏
    UNITY_SAMPLE_SHADOW_PROJ：进行阴影投影计算的宏
    */

    #if !defined(SHADER_API_GLES)
        // all platforms except GLES2.0 have built-in shadow comparison samplers
        // 除GLES2.0之外的所有平台均，内置阴影比较采样器
        ...
        #define UNITY_SAMPLE_SHADOW_PROJ(tex,coord) tex.SampleCmpLevelZero (sampler##tex,(coord).xy/(coord).w,(coord).z/(coord).w)
            ...
            // GLSL不支持textureLod（samplerCubeShadow，...）。 
            // GLES2没有对samplerCubeShadow的核心支持，因此我们将其忽略
        ...
        // 兜底方案 / 没有内置的阴影贴图比较采样 -> 常规纹理采样并进行手动深度比较
        // 如果不支持原生的阴影操作函数，则使用和操作普通 2D 纹理类似的函数，
        // 宏 SAMPLE_DEPTH_TEXTURE 包装了这些普通 2D 纹理函数
    ...
    /*
    可以分别声明纹理和采样器的宏。 
    对于具有独立采样器和纹理的平台（例如DX11），我们希望保留采样器。
    -UNITY_DECLARE_TEX * _NOSAMPLER：声明纹理，而没有采样器。
    -UNITY_SAMPLE_TEX * _SAMPLER：使用另一个纹理的采样器对纹理进行采样。必须在当前着色器中实际使用另一个纹理，否则将无法设置正确的采样器。
    */
        ...
        // DX11样式的HLSL语法； 单独的纹理和采样器
        //
        // 注意：对于HLSLcc，我们有特殊的单位特定语法来传递采样器精度信息。
        //
        // 注意：对于曲面着色器分析，在完成非mojoshader分析的一部分时进入DX11语法路径，
        // 这允许曲面着色器使用_NOSAMPLER和类似的宏，而无需使用采样器寄存器。
        // 请勿在mojoshader部分中执行此操作，因为那只能解析DX9样式的HLSL。
        ...
        #define UNITY_SAMPLE_TEX2D_SAMPLER(tex,samplertex,coord) tex.Sample (sampler##samplertex,coord)
            ...
            // GL Core doesn't have the _half mangling, the rest of them do.
            // GL Core没有_half修改，其余的都有。
    ...
    // HLSL attributes
    // 分支流程控制相关的宏，只在 HLSL语言中有定义，其在其他的着色器语言中是一个空宏
    #if defined(UNITY_COMPILER_HLSL)
        #define UNITY_BRANCH    [branch]        // 在 if 条件式前使用 当 if 条件满足时，执行条件满足时的语句块，否则，就执行条件不满足时的语句块。
        #define UNITY_FLATTEN   [flatten]       // 在 if 条件式前使用 if 条件满足与否的两部分代码都执行一遍，执行完毕之后再选择其中一个结果
        #define UNITY_UNROLL    [unroll]        // 在 for 条件式前使用 表示shader在编译时不进行循环展开检查
        #define UNITY_LOOP      [loop]          // 在 for 条件式前使用 表示编译时尝试展开循环，x为尝试展开循环的最大次数
        #define UNITY_FASTOPT   [fastopt]       // 编译时不进行循环展开，和LOOP类似，但不太清楚具体区别
    #else
        #define UNITY_BRANCH
