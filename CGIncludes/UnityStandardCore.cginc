...
// 要么得到静态/动态（实时）光照贴图的UV纹理坐标，
// 要么利用球谐函数得到作用再本物体上的光线RGB值
inline half4 VertexGIForward(VertexInput v, float3 posWorld, half3 normalWorld)
{
    half4 ambientOrLightmapUV = 0;
// Static lightmaps
#ifdef LIGHTMAP_ON // 启用静态光照贴图
    ambientOrLightmapUV.xy = v.uv1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
    ambientOrLightmapUV.zw = 0;

// 启用了 UNITY_SHOULD_SAMPLE_SH 宏，表示不对静态/动态的光照贴图进行采样，而对动态的物体进行采样
// Sample light probe for Dynamic objects only (no static or dynamic lightmaps)
// 仅为动态物体采样光照探针，不对静态/动态光照贴图进行采样
#elif UNITY_SHOULD_SAMPLE_SH

#ifdef VERTEXLIGHT_ON       // 当开启了逐顶点光照
...
// 若定义了 VERTEXLIGHT_ON DYNAMICLIGHTMAP_ON 宏[开启动态（实时）光照贴图]则给变量的 z、w 分量赋值
...
void fragDeferred(
    ...
    : SV_Target3 // RT3: emission (rgb), --unused-- (a)
      ...
      ,
      out half4 outShadowMask
    : SV_Target4 // RT4: shadowmask (rgba)
#endif
)
{
#if (SHADER_TARGET < 30)
